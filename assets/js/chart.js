...
var three_month_ago = new Date(current_time.getTime());
three_month_ago = three_month_ago.setMonth(three_month_ago.getMonth() - 3);
three_month_ago = (three_month_ago / 1000 | 0);
var timestamp_current_time = +new Date(current_time.getTime());
timestamp_current_time = (timestamp_current_time / 1000 | 0);
...
xmlHttp.open("GET", 'https://api.nobitex1.ir/market/udf/history?symbol=BTCIRT&resolution=720&from=' + three_month_ago + '&to=' + timestamp_current_time, false); // false for synchronous request
xmlHttp.send(null);
...
var volume = []
for (var key in open) {
    data[key] = {time: time[key], open: open[key], close: close[key], high: high[key], low: low[key]};
}
;
...
var currentBusinessDay = timestamp_current_time;
...
var one_hour_before = new Date();
one_hour_before = one_hour_before.setHours(one_hour_before.getHours() - 1);
one_hour_before = (one_hour_before / 1000 | 0);

var timestamp_last_candle = +new Date();
timestamp_last_candle = (timestamp_last_candle / 1000 | 0);

function mergeTickToBar(price) {
    var httpLastCandle = new XMLHttpRequest();
    httpLastCandle.open("GET", 'https://api.nobitex1.ir/market/udf/history?symbol=BTCIRT&resolution=720&from=' + one_hour_before + '&to=' + timestamp_last_candle, false); // false for synchronous request
    //
    // alert('https://api.nobitex1.ir/market/udf/history?symbol=BTCIRT&resolution=720&from=' + one_hour_before + '&to=' + timestamp_last_candle);
    httpLastCandle.send(null);
    var result = httpLastCandle.responseText;
    result = JSON.parse(result);
    var open2 = result['o'];
    var close2 = result['c'];
    var high2 = result['h'];
    ...
    var time2 = result['t'];
    var value2 = result['v'];
    currentBar.open = open2;
    currentBar.close = close2;
    currentBar.high = high2;
    currentBar.low = low2;
...
function nextBusinessDay(time) {
    ...
    // };
    var x = current_time;
    return x.setHours(current_time.getHours() + 1);
}


setInterval(function () {
   var solidNextBarNowTime = new Date();
    var nextBarOneMinAgo = new Date();
    nextBarOneMinAgo.setMinutes(nextBarOneMinAgo.getMinutes() - 1);
    var nextBarOneMinHour = nextBarOneMinAgo.getHours();
    var nextBarNowHour = solidNextBarNowTime.getHours();
    ...
    if ( parseInt(nextBarNowHour)> parseInt(nextBarOneMinHour)) {
        // move to next bar
        currentIndex++;
